---
title: "Visual and overview of GISAID mutation counts data + covariates: early period "
author: "T. Bataillon "
...
data100_annotated<- readRDS( "output/data_100pb_pangolin_period1_withCovs.rds")
...
data100_annotated %>%
  select(gene, gene_bin, type, unique_mutations, W.G.dS) %>%
  slice(400:410) %>%
  knitr::kable(digits = 2)
...
ggplot(aes(x=ii.CD8.max, y=unique_mutations, size=max_genomes_mutated, color=gene_name)) + 
  ...
  facet_wrap(~ gene + pangolin_lineage , ncol = 2, scales = "free_y")+
