---
title: "Implementing Models of SAR-CoV-2 mutation counts: explorind dS versus syn counts "
author: "T Bataillon"
...
*NB to be removed as version tracking makes this obsolete .. ;) * trying with period 3 as example
...
ggplot(data = df_muts_and_covs,
  ...
  facet_wrap(~ pangolin_lineage + period+ type , ncol = 2, strip.position = "top")+
...
df_muts_and_covs %>%
  ...
  geom_smooth()+
  facet_wrap(~pangolin_lineage + period, ncol = 3)
...
```{r}
ci_alpha <- 0.05/(307*3) #hardcoded here to be updated
...
by_type_m1 <- 
  by_type_m1 %>%
  mutate(categ = ifelse( (unique_mutations > UCB0.999972855591748 ), "HO", "HA"))
names(by_type_m1)
```
