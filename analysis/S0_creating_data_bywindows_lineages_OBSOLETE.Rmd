...
* We output counts as rds files for each lineage separately



```{r hardcode_data_path}
...
names(genomes_md)
head(genomes_md) %>% knitr::kable(digits = 3)
```
...
names(mutations)
names(genomes_md)
names(gene_positions_md)
names(gene_positions_md)[1]<-"gene" # Renaming the gene column to make it compatible with the mutation tibble
```
...
## Pangolin lineages sample size distribution
...
Note this new version: referencing according to Susan data integrataion
We select a subset of lineages `to_extract`:

```{r}
to_extract<-c("B.1.177", "B.1.5", "D.2", "A.1",  "B.1.1.7", "B.1.351", "P.1")
```

AS check 
we extract sets of genomes mayching the two time periods of Angela's original parse 
we keep wins having up to 1 mutation.
